...
class AsyncServiceHelper
{
    ...
    public static boolean initOpenCV2(String Version,
        ...
        Intent implicitIntent = new Intent("org.opencv.engine.BIND");
        List<ResolveInfo> resolveInfos = pm.queryIntentServices(implicitIntent, 0);

        // Is somebody else trying to intercept our IAP call?
        if (resolveInfos == null || resolveInfos.isEmpty()) {
            Log.i(TAG, "queryIntentServices got 0");
            //      AppContext.unbindService(helper.mServiceConnection);
            InstallService(AppContext, Callback);
            return false;
        } else if (resolveInfos.size() > 1) {
            Log.i(TAG, "queryIntentServices got more than 1");
            return false;
        }
        ...
        if (AppContext.bindService(iapIntent, helper.mServiceConnection, Context.BIND_AUTO_CREATE)) {
            return true;
        } else {
            AppContext.unbindService(helper.mServiceConnection);
            InstallService(AppContext, Callback);
